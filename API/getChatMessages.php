<?php
   ...
   // $sql_getData = "SELECT `chat_id`, `From`, `To`, `Data`, `chat_started`, `status`, `last_massage`, `last_updated` FROM `chat` WHERE `From` = $userid And `To` = $friend_id";
     ...
     $sql_getData = "SELECT `m_id`, `sender_id`, `reciever_id`, `Data`, `last_message`, `is_read`, `status`, `created_at`, `updated_at` FROM `messages` WHERE `reciever_id` = '$friend_id' AND `sender_id` = '$userid'";
