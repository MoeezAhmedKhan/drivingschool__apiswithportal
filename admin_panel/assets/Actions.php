...
if(isset($_POST['spname']) && isset($_POST['spicon']) && isset($_POST['spicontype']))
{
        print_r($_POST);
...
if(isset($_GET['FunctionName'])){
 if($_GET['FunctionName']=="ToggleCampaignPro"){
     ...
     $result = mysqli_query($conn,$sql);
      ...
      $id = $_GET['id'];
     $sql = "DELETE FROM `specials_cateogries` WHERE `id`=$id";
     $result = mysqli_query($conn,$sql);
         if($result){
             echo "Done";
         }
    ...
    //   echo 1;
      ...
      $result = mysqli_query($conn,$sql);
      if($result){
        header('Location: ../manageowners.php?Message=Sucessfully updated details.');
      }
