<?php
            ...
            //$chatArray = array();
                ...
                `is_read`='notread',`status`='newmessage' WHERE `sender_id`='$friend_id' AND `reciever_id`='$user_id'";
		   	  ...
		   	  //  if you want to update data in inbox.
		   	    ...
		   	    WHERE `sender_id`= '$user_id' and `reciever_id`= '$friend_id'";
   ...
   else
   {
    ...
    if(mysqli_num_rows($exec_chk_ibox) == 0){
		   	    ...
		   	    `date`) VALUES ('$user_id','$friend_id','$user_massage','unread','$currentTime')";
		   	    mysqli_query($conn,$sqlInsertInbox);
		   	    
		   	    $sqlInsertInbox2 = "INSERT INTO `inbox`(`sender_id`, `reciever_id`, `last_message`, `status`,
		   	    `date`) VALUES ('$friend_id','$user_id','$user_massage','read','$currentTime')";
 ...
 function sendMessage($idd, $uzer_id)
 {
    include('connection.php');
    $subject = '';
    $sqltaskMembers = "SELECT `id`, `First_name`, `Last_name`, `social_id`, `role_id`, `email`, `Phone`, `City`,
    `Address`, `Zip_code`, `Driving_license_front_image`, `Driving_license_rear_image`, `password`, `Status`,
    `notification_token`, `created_at` FROM `users` WHERE  `id` = '$idd'";
                        
    $taskMembers = mysqli_query($conn,$sqltaskMembers);
        
    $sqlother = "SELECT `id`, `First_name`, `Last_name`, `social_id`, `role_id`, `email`, `Phone`, `City`,
    `Address`, `Zip_code`, `Driving_license_front_image`, `Driving_license_rear_image`, `password`, `Status`,
    `notification_token`, `created_at` FROM `users` WHERE  `id` = '$uzer_id'";        
    $exec_other = mysqli_query($conn,$sqlother);   
    $other = mysqli_fetch_array($exec_other);
    $subject =  $other['First_name'];
    ...
    $fields = array
    (
        'app_id' => "9b615470-0d1d-4246-a88d-50487f148a35",
