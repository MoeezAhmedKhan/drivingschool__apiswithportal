...
if(isset($_POST['btn']))
{
  ...
  $file_type = $_FILES['frontimage']['type']; 
  $file_size = $_FILES['frontimage']['size']; 
  $temp_loc = $_FILES['frontimage']['tmp_name']; 

  
  /*rear image*/
  
  $target_file2 = $target_dir . basename($_FILES["rearimage"]["name"]);
  $imageFile2Type = strtolower(pathinfo($target_file2,PATHINFO_EXTENSION));
   $filewithnewname2 =  date("Ymdi")."LIC.".$imageFile2Type;   
  

  $rear_type = $_FILES['rearimage']['type']; 
  $rear_size = $_FILES['rearimage']['size']; 
  $rear_loc = $_FILES['rearimage']['tmp_name']; 
  ...
  $check_1 = "select * from users where email = '$email' and Phone = '$phone'";
  $check_exec_1 = mysqli_query($conn,$check_1);
  $check_row_1 = mysqli_num_rows($check_exec_1);
  if($check_row_1 == 0)
  {
  
  
      if(strtolower($file_type) == "image/jpg" || strtolower($file_type) == "image/png" || strtolower($file_type) == "image/jpeg" || strtolower($file_type) == "image/gif")
      {
          if(strtolower($rear_type) == "image/jpg" || strtolower($rear_type) == "image/png" || strtolower($rear_type) == "image/jpeg" || strtolower($rear_type) == "image/gif")
          {
          
              if($file_size <= 5000000) /*front 5mb*/
              {
                  if($rear_size <= 5000000) /*rear 5mb*/
                  {
                  
                      $query1 = "INSERT INTO `users`(`First_name`, `Last_name`, `role_id`, `email`, `Phone`, `City`, `Address`, 
                      `Zip_code`,`Driving_license_front_image`, `Driving_license_rear_image`, `password`)
                      VALUES ('$firstname','$lastname','1','$email','$phone','$city','$address','$zipcode','$filewithnewname'
                      ,'$filewithnewname2','$password')";
                      $run1 = mysqli_query($conn,$query1);
                      if($run1)
                      {
                          move_uploaded_file($temp_loc,$target_dir.$filewithnewname);
                          move_uploaded_file($rear_loc,$target_dir.$filewithnewname2);
                          
                          header("Location:../create_teachers_account.php?Massage=Record Inserted");
                          
                      }
                      else
                      {
                          header("Location:../create_teachers_account.php?Massage=Record Insert Failed");
                      }
                  
                  }
                  else
                  {
                     header("Location:../create_teachers_account.php?Massage=Rear Image Should be less than 5 Mb");
                  }
              
              }
              else
              {
                   header("Location:../create_teachers_account.php?Massage=Front Image Should be less than 5 Mb");
              }
          
          }
          else
          {
               header("Location:../create_teachers_account.php?Massage=Rear file extension is wrong || You can insert jpg, png, jpeg, and gif");
          }
      }
      else
      {
         header("Location:../create_teachers_account.php?Massage=Front file extension is wrong || You can insert jpg, png, jpeg, and gif");
      }
  }
  else
  {
     header("Location:../create_teachers_account.php?Massage=Sorry your record is already exist || please insert different record");
  }
...
if(isset($_POST['lesson_btn']))
{
  ...
  $check_2 = "SELECT * FROM `lessons` where l_title = '$title'";
  $check_exec_2 = mysqli_query($conn,$check_2);
  $check_row_2 = mysqli_num_rows($check_exec_2);
  if($check_row_2 == 0)
  {
          
        $query2 = "INSERT INTO `lessons`(`l_title`, `color`) VALUES ('$title','$color')";
        $run2 = mysqli_query($conn,$query2);
        if($run2)
        {
           header("Location:../add_lesson.php?Massage=Sucessfully added new area.");
        }
        else
        {
           header("Location:../add_lesson.php?Massage=Failed added new news.");
  ...
  {
      ?>
            <script>alert("Sorry your record is already exist please insert different record");
...
if(isset($_POST['lesson_hours_btn']))
{

  $lession_id = $_POST['lesson']; 
  $date = $_POST['date']; 
  $lession_start = date_create($_POST['start']);
  $lession_end = date_create($_POST['end']);
  $lession_price = $_POST['price'];
  
  $s = date_format($lession_start, "g:i A");
  $e = date_format($lession_end, "g:i A");
 
  require_once('../assets/connection.php');
  
//   $check_3 = "SELECT * FROM `classes` WHERE start_time = '$s'";
  
//   $check_exec_3 = mysqli_query($conn,$check_3);
//   $check_row_3 = mysqli_num_rows($check_exec_3);
 
//   if($check_row_3 == 0)
//   {
        //   $check_inner = "SELECT * FROM `classes` WHERE end_time = '$e'";
        //   $check_exec_inner = mysqli_query($conn,$check_inner);
        //   $check_row_inner = mysqli_num_rows($check_exec_inner);
        //   if($check_row_inner == 0)
        //   {
      
                $query3 = "INSERT INTO `classes`(`teacher_id`, `lesson_id`, `date`, `start_time`, `end_time`)
                VALUES (13,'$lession_id','$date','$s','$e')";
                $run3 = mysqli_query($conn,$query3);
                if($run3)
                {
                   header("Location:../add_lesson_hours.php?Massage=Sucessfully added new news.");
                }
                else
                {
                    ?>
                    <script>alert("there was an some error");window.location.href="../add_lesson_hours.php"</script>
                    <?php 
                }
        //   }
        //   else
        //   {
        //       >
        //             <script>alert("Sorry End time is already exist please insert different time");
        //             window.location.href = "../add_lesson_hours.php";
        //             </script>
        //     <?php
        //   }
...
//   {
//       >
//             <script>alert("Sorry Start time is already exist please insert different time");
//             window.location.href = "../add_lesson_hours.php";
//             </script>
//      <?php
//   }
...
if(isset($_POST['terms_btn']))
{
  ...
  $check_4 = "SELECT * FROM `terms_cond` where Title = '$t_title'";
  $check_exec_4 = mysqli_query($conn,$check_4);
  $check_row_4 = mysqli_num_rows($check_exec_4);
  if($check_row_4 == 0)
  {
          
        $query4 = "INSERT INTO `terms_cond`(`Title`, `Description`) VALUES 
        ('$t_title','$t_desc')";
        $run4 = mysqli_query($conn,$query4);
        if($run4)
        {
           header("Location:../terms.php?Massage=Sucessfully added terms and condition.");
        }
        else
        {
           header("Location:../terms.php?Error=Terms and conditionS are not insert.");
        }
  ...
  else
  {
      ...
      ?>
            <script>alert("Sorry your Term is already exist please insert different Term");
...
if(isset($_POST['package_btn']))
{
  ...
  $check_2 = "SELECT * FROM `Packages` where Title = '$title'";
  $check_exec_2 = mysqli_query($conn,$check_2);
  $check_row_2 = mysqli_num_rows($check_exec_2);
  if($check_row_2 == 0)
  {
          
        $query5 = "INSERT INTO `Packages`(`Title`, `Hours`, `Price`) VALUES ('$title','$hours','$price')";
        $run5 = mysqli_query($conn,$query5);
        if($run5)
        {
           header("Location:../addpackages.php?Massage=Sucessfully added new area.");
        }
        else
        {
           header("Location:../addpackages.php?Massage=Failed added new news.");
  ...
  {
      ?>
            <script>alert("Sorry your package is already exist please insert different package");
            window.location.href = "../add_lesson.php";
            </script>
     <?php
  }
